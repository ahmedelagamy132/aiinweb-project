...
export function AgentPanel() {
    ...
    const [startLocation, setStartLocation] = useState('San Francisco Depot');
    ...
    /**
     * Handle map click - add new delivery stop
     */
        ...
        try {
            ...
            const routeRequest = {
                ...
                stops: stops.map(stop => ({
                    ...
                    time_window_start: stop.time_window_start || undefined,
                    time_window_end: stop.time_window_end || undefined,
    ...
    return (
        <div style={{ display: 'flex', height: '100%', gap: '1.5rem', padding: '0.5rem' }}>
            {/* Map Section */}
            <div style={{ flex: '2', minWidth: '0', display: 'flex', flexDirection: 'column' }}>
                <div style={{ marginBottom: '1rem', padding: '1rem', background: 'linear-gradient(135deg, #1a1f2e 0%, #2a2f3a 100%)', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.3)' }}>
                    ...
                    <p style={{ margin: '0', color: '#a0aec0', fontSize: '0.95rem' }}>Click on the map to add delivery stops, then configure details in the sidebar.</p>
                </div>
            ...
            <div style={{ flex: '1', minWidth: '360px', maxWidth: '420px', overflowY: 'auto', paddingRight: '0.5rem' }}>
                ...
                <div style={{ marginBottom: '1rem', padding: '1.25rem', background: '#1e2330', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.2)', border: '1px solid #2a3142' }}>
                    ...
                    <div className="form-group" style={{ marginBottom: '0.75rem' }}>
                        <label htmlFor="startLocation" style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.9rem', fontWeight: '500', color: '#cbd5e0' }}>Start Location</label>
                        <input
                            type="text"
                            id="startLocation"
                            value={startLocation}
                            onChange={(e) => setStartLocation(e.target.value)}
                            placeholder="Depot or Warehouse"
                            style={{ width: '100%', padding: '0.6rem', fontSize: '0.95rem', borderRadius: '6px', border: '1px solid #3a3f4a', background: '#0f1419', color: '#e2e8f0' }}
                        />
                    </div>
                    <div className="form-group" style={{ marginBottom: '0.75rem' }}>
                        <label htmlFor="plannedStartTime" style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.9rem', fontWeight: '500', color: '#cbd5e0' }}>Planned Start Time</label>
                        <input
                            type="datetime-local"
                            id="plannedStartTime"
                ...
                {result && (
                    <div className="content-box" style={{ marginTop: '1rem' }}>
                        ...
                        {result.issues && result.issues.length > 0 && (
                            <div style={{ marginBottom: '1rem' }}>
                                <h4 style={{ fontSize: '0.9rem', marginBottom: '0.5rem' }}>Issues:</h4>
                                <ul style={{ paddingLeft: '1.5rem', fontSize: '0.9rem' }}>
                                    {result.issues.map((issue, i) => (
                                        <li key={i} style={{ marginBottom: '0.25rem', color: '#ff6b6b' }}>{issue}</li>
                                    ))}
                                </ul>
                            </div>
                        )}
                        ...
                        {result.recommendations && result.recommendations.length > 0 && (
                            <div style={{ marginBottom: '1rem' }}>
                                <h4 style={{ fontSize: '0.9rem', marginBottom: '0.5rem' }}>Recommendations:</h4>
                                <ul style={{ paddingLeft: '1.5rem', fontSize: '0.9rem' }}>
                                    {result.recommendations.map((rec, i) => (
                                        <li key={i} style={{ marginBottom: '0.25rem', color: '#60a5fa' }}>{rec}</li>
                                    ))}
                                </ul>
                            </div>
                        )}
                        ...
                        {result.optimized_stop_order && (
                            <div style={{ marginBottom: '1rem' }}>
                                <h4 style={{ fontSize: '0.9rem', marginBottom: '0.5rem' }}>Optimized Order:</h4>
                                <div style={{ fontSize: '0.85rem', color: '#8b92a0' }}>
                        ...
                        {(result.estimated_duration_hours || result.estimated_distance_km) && (
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.85rem' }}>
                                {result.estimated_duration_hours && (
                                    <div>
                                        <strong>Duration:</strong> {result.estimated_duration_hours.toFixed(1)}h
                                    </div>
                                )}
                                {result.estimated_distance_km && (
                                    <div>
                                        <strong>Distance:</strong> {result.estimated_distance_km.toFixed(1)}km
                                    </div>
                                )}
                            </div>
                        )}
